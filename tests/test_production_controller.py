...
class TestProductionControllerCompleteJob:
    def test_complete_job_calls_service_with_job_id(self, controller, mock_service, mock_view):
        mock_view.show_complete_prompt.return_value = "J001"
    ...
    def test_complete_job_shows_success(self, controller, mock_service, mock_view):
        mock_view.show_complete_prompt.return_value = "J001"
        mock_service.complete_job.return_value = Order(
            order_id="O001", sample_id="S001", customer_name="Lab", quantity=10, status=OrderStatus.CONFIRMED
        )
    ...
    def test_complete_job_shows_error_on_failure(self, controller, mock_service, mock_view):
        mock_view.show_complete_prompt.return_value = "BAD_ID"
        mock_service.complete_job.side_effect = ValueError("Job not found")
        controller.complete_job()
        mock_view.show_error.assert_called_once()

    def test_complete_job_skips_when_empty_input(self, controller, mock_service, mock_view):
        mock_view.show_complete_prompt.return_value = ""
        controller.complete_job()
